...
class IndexController extends AbstractActionController{
    ...
    public function onDispatch(MvcEvent $e){
        ...
        // check qrcode
        $this->qrCode = $this->params()->fromQuery('s', '');
        //echo $this->qrCode; die();
        if($this->qrCode == ''){
            $arrFnd = explode('=', $_SERVER['REQUEST_URI']);
            $this->qrCode = isset($arrFnd[1]) ? substr($arrFnd[1],0,8) : '';
        }
        //echo $this->qrCode; die();
        if($this->qrCode == ''){
            die('Không tìm thấy qrcode này');
        }
        $this->currentCode = $this->entityCodes->fetchRowAsQrcode($this->qrCode, COMPANY_ID);
    ...
    public function indexAction(){
        ...
        $url = FULL_SERVER_NAME . "/api/index/?Command_Code=QRCODE&User_ID=" . "938350786";
        ...
        $this->replaceLayout([
            "product"   => array_merge((array)$arrProduct, (array)$datasProduct),
            "agent"     => array_merge((array)$arrAgent, (array)$datasAgent),
    ...
    public function layout01Action(){
        ...
        if($request->isPost()){
            ...
            if($isValid){
				...
				$type_agent = str_replace(' ', '%20', $valuePost['type_agent']);
...
//                 echo "-" . $dataSend . "-";
    ...
    public function layout08Action(){
        ...
        if($this->defineCompanyId == 'HAR'){
            header("Location: http://www.harose.vn/");
        }
        die();
    }
