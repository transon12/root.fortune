...
class Orders{
    ...
    public function fetchAlls($options = null, $arr = null){
        ...
        //         $select->where("$seString"); 
        //     }else{
        //         $select->where("id = ''");
        //     }
        // }
        if(\Admin\Service\Authentication::getId() != "1" && \Admin\Service\Authentication::getUsername() != "thanhtam" && \Admin\Service\Authentication::getUsername() != "ngocvt" ){
            if(isset($arr) && !empty($arr)){
                $i = 0;
                foreach($arr as $item){
                    $se[$i] = "company_id = '".$item."'";
                    $i++;
                }
                $seString = implode(' or ',$se);
                $select->where("($seString)");
