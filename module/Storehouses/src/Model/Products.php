...
class Products{
    ...
    public function fetchAllOptions02($options = null){
        ...
        $res = [];
        foreach($re as $item){
            $dataCode = json_decode($item["data_codes"], true);
            $res[$item['id']] = $item['name'] . ' - Ký hiệu mã: ' . $dataCode['prefix_code'] . ' - Ký hiệu serial: ' . $dataCode['prefix_serial'] . ' - Serial bắt đầu: ' . $item['serial_begin'];
        }
        return $res;
    }
    ...
    public function fetchAllOptions04($options = null){
        $adapter = $this->adapter;
        $sql    = new Sql($this->adapter);
        $select = $sql->select();
        $select->from($this->_name);
        ...
        foreach($re as $item){
            $dataCode = json_decode($item["data_codes"], true);
            $res[$item['id']] = $item['name'] . ' - Ký hiệu mã: ' . $dataCode['prefix_code'] . ' - Ký hiệu serial: ' . $dataCode['prefix_serial'];
        }
        return $res;
    }
    ...
    public function fetchRowWithCompanyId($id){
        $adapter = $this->adapter;
        $sql    = new Sql($this->adapter);
        $select = $sql->select();
        $select->from($this->_name);
